...
class DDPM(BaseModel):
    def __init__(self, opt):
        ...
        if self.opt['phase'] == 'test':
            ...
            self.netG.load_state_dict(torch.load(self.opt['path']['resume_state']), strict=False)
            print('loaded ', self.opt['path']['resume_state'])
        else:
            if opt['dist']:
    ...
    def feed_data(self, data):
        ...
        if self.opt['dist']:
            ...
            dic['LQ'] = data.to(self.local_rank)
            dic['GT'] = data.to(self.local_rank)
            # dic['Ill'] = data['Ill']
            self.data = dic
        else:
            # dic['LQ'] = data['LQ']
    ...
    def optimize_parameters(self):
        ...
        if not self.opt['uncertainty_train']:
            if self.opt['dist']:
                # l_pix, l_svd = self.netG(self.data, self.netGU.module.denoise_fn)
                l_pix = self.netG(self.data, self.netGU.module.denoise_fn)
            else:
                # l_pix, l_svd = self.netG(self.data, self.netGU.denoise_fn)
                l_pix = self.netG(self.data, self.netGU.denoise_fn)
            ...
            # l_svd = l_svd.sum()/int(b*num_clusters)
            loss = l_pix 
            loss.backward()
            self.optG.step()
