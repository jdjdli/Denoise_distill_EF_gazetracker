...
def train_one_epoch(model, optimizer, data_loader, device, epoch, teacher1, teacher2, teacher3, teacher4):
    model.train()
    teacher1.eval()
    teacher2.eval()
    teacher3.eval()
    ...
    for step, data in enumerate(data_loader):
        frames, voxels, frames_tmp_1, voxels_tmp_1, frames_tmp_2, voxels_tmp_2, frames_tmp_3, voxels_tmp_3, frames_tmp_4, voxels_tmp_4, labels, weight_label = data
        frames, voxels, frames_tmp_1, voxels_tmp_1, frames_tmp_2, voxels_tmp_2, frames_tmp_3, voxels_tmp_3, frames_tmp_4, voxels_tmp_4, labels, weight_label = torch.autograd.Variable(frames), torch.autograd.Variable(voxels), torch.autograd.Variable(frames_tmp_1), torch.autograd.Variable(voxels_tmp_1), torch.autograd.Variable(frames_tmp_2), torch.autograd.Variable(voxels_tmp_2), torch.autograd.Variable(frames_tmp_3), torch.autograd.Variable(voxels_tmp_3),torch.autograd.Variable(frames_tmp_4), torch.autograd.Variable(voxels_tmp_4), torch.autograd.Variable(labels), torch.autograd.Variable(weight_label)
        sample_num += frames.shape[0]
        # pred_tmp = model(frames_tmp.to(device), voxels_tmp.to(device))
        pred = model(frames.to(device), voxels.to(device), frames_tmp_1.to(device), voxels_tmp_1.to(device), frames_tmp_2.to(device), voxels_tmp_2.to(device), frames_tmp_3.to(device), voxels_tmp_3.to(device), frames_tmp_4.to(device), voxels_tmp_4.to(device))
        ...
        avg_teacher_logits = (teacher1_logits[2] + teacher2_logits[2] + teacher3_logits[2] + teacher4_logits[2]) / 4
        
        avg_teacher_logits_feature = (teacher1_logits[3] + teacher2_logits[3] + teacher3_logits[3] + teacher4_logits[3]) / 4
        ...
        loss = alpha * soft_loss + (1 - alpha) * hard_loss + 0.98 * soft_loss_1
...
def evaluate(model, data_loader, device, epoch):
    ...
    for step, data in enumerate(data_loader):
        ...
        batch_numpy= pred[4].cpu().detach().numpy()
        orig_frames = frames.cpu().detach().numpy()
        # new_feature = []
        for i in range(pred[4].size(dim=0)):
            scipy.io.savemat(f'feature_map_process/feature_map_{i}_orig_data.mat', {f'feature_map_{i}_orig_data': batch_numpy[i], f'orig_{i}_frame_data': orig_frames[i]})
