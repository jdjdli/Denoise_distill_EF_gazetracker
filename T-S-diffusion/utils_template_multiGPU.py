            ...
            # print(len(val))
                ...
                for i in range(len(frames)):
                    ...
                    if frames[i] not in val:
                        ...
                        train_position_label.append(abs_position[i])
                    ...
                    # train_template_end_voxels_path.append(voxels[-1])
                        ...
                        val_position_label.append(abs_position[i])
...
def train_one_epoch(model, optimizer, data_loader, device, epoch, teacher1, teacher2, teacher3, teacher4, teacher5, diffusion):
    ...
    average_euclidean_distance_num = torch.zeros(1).to(device) #误差
    ...
    accumulate_steps = 128  # 定义要累积的步骤数量
    ...
    #     loss_new = torch.norm(output_teacher - output_student, p=2)
        ...
        loss = nn.L1Loss()(pred[5], labels.float().to(device))
        ...
        # 平均欧氏距离
        average_euclidean_distance = torch.mean(torch.sqrt(torch.sum((labels.float().to(device) - pred[5]) ** 2, dim=1)))

        # average_euclidean_distance_num += average_euclidean_distance
    ...
    return accu_loss.item() / (step + 1), average_euclidean_distance.item(), mlp_loss.item() / (step + 1)
...
def evaluate(model, data_loader, device, epoch):
    ...
    accu_loss = torch.zeros(1).to(device)  # 累计损失
    mlp_loss = torch.zeros(1).to(device)
    average_euclidean_distance_num = torch.zeros(1).to(device)
